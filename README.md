...
로그인 계정 시스템 없이도, 각자 자신의 Notion Integration Token·Database ID를 ⚙️ 설정에 넣어두면 **그 사람의 노트는 그 사람의 Notion에만 쓰이고 읽혀요.** 설정 모달의 **"Notion에서 데이터 불러오기"** 버튼을 누르면(자동 동기화가 아니라 누를 때만 실행돼요):

1. 입력한 Notion 데이터베이스를 조회해 최근 노트(최대 20개)를 가져와요.
2. 이 브라우저에 있던 노트를 Notion 내용으로 **덮어써요** — 노트를 저장할 때마다 이미 Notion으로 보내고 있으니 최신 상태는 보통 Notion에 있다는 전제예요.
3. 불러온 노트는 그래프·노트 목록에 별도 작업 없이 바로 반영돼요(둘 다 같은 localStorage를 보고 그리기 때문).

다른 기기·브라우저에서도 같은 Notion 값을 넣고 이 버튼을 누르면 노트가 그대로 이어져요 — 계정 로그인 없이 "내 데이터를 내 Notion에 보관하고 다시 불러오는" 가벼운 개인별 저장 방식이에요.
...
│   └── notion_fetch.py # "Notion에서 데이터 불러오기" — Notion 데이터베이스를 조회해 노트로 변환
├── requirements.txt
└── vercel.json
```
