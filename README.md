...
- **아이디어 그래프**: 노트를 추가하면 저장된 노트끼리 AI가 뽑은 키워드로 자동 연결되고, "내 노트 AI 분석"으로 의미 기반 연결(이유 포함)을 더할 수 있어요. 결과는 옵시디언용 마크다운(`[[내부링크]]` 포함)으로 복사할 수 있어요.
  - **검색**: 그래프 카드 오른쪽 위 돋보기(🔍)를 누르면 입력창이 열리고, 입력하는 대로 제목·내용이 일치하는 노드만 밝아져요.
  - **편집**: 노드를 클릭해 연 SELECTED NOTE 패널에서 "편집" 버튼으로 제목·내용·키워드를 바로 고칠 수 있어요. 저장하면 그래프가 새 키워드 기준으로 다시 연결돼요.
  - **다시 정리하기**: 예전에 저장한 노트들의 키워드를 지금 기준(첫 키워드는 항상 분야 카테고리)으로 AI가 다시 뽑아, 흩어져 있던 노트도 같은 분야끼리 새로 연결되게 해요.
...
- 그래프 검색·편집·재정리: 노트를 찾고, 그 자리에서 고치고, 예전 노트도 새 기준으로 다시 연결할 수 있어요 (위 [아이디어 그래프](#주요-기능) 참고).
